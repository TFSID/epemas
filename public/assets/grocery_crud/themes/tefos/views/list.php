...
?><div class="Div table-responsive" >
		<table class="table text-wrap"  cellspacing="0" cellpadding="0" border="0" id="flex1">
		<thead>
			<tr class=''>
				<?php foreach($columns as $column){?>
				<th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7" width='<?php echo $column_width?>%'>
...
<?php foreach($list as $num_row => $row){ ?>        
		<tr  <?php if($num_row % 2 == 1){?>class=""<?php }?>>
			...
			<td width='<?php echo $column_width?>%' class='<?php if(isset($order_by[0]) &&  $column->field_name == $order_by[0]){?>sorted<?php }?>'>
				<div class='text-left text-wrap'><?php echo $row->{$column->field_name} != '' ? $row->{$column->field_name} : '&nbsp;' ; ?></div>
			</td>
			<?php }?>
